...
// ▼▼▼ SSG: 記事リストデータ取得 (ローカルファイル読み込みはServer SideなのでOK) ▼▼▼
...
// ▼▼▼ SecureImage (Gateway経由・相対パスのみ) ▼▼▼
// フロントは認証もBlobも扱わない。「関所(/)」に「お土産(content_id)」を持って画像をもらうだけ。
const SecureImage = ({ contentId, alt, className }) => {
  if (!contentId) return <div className="no-image">Loading...</div>;

  // Gateway Workerへリクエスト (type=image)
  const src = `/?content_id=${contentId}&type=image`;
...
export default function Articles({ posts }) {
  ...
  const HERO_ID = "IL041";
  const heroImageSrc = `/image/${HERO_ID}`;
